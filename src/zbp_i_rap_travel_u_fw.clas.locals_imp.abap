...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD update.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).
      ...
      ELSE.
        " fill failed return structure to the framework
        APPEND VALUE #( travelid = legacy_entity_in-travel_id
                        %msg     = new_message( id = messages[ 1  ]-msgid
                                                number = messages[ 1 ]-msgno
                                                v1     = messages[ 1 ]-msgv1
  ...
  METHOD read.
    DATA:
      legacy_entity_out TYPE /dmo/travel,
          ...
          APPEND VALUE #( travelid = key-TravelId
                          %fail-cause = COND #(
                                         WHEN
                                           ...
                                           THEN
                                             if_abap_behv=>cause-not_found
                                           ELSE
                                             if_abap_behv=>cause-unspecific )
                         )
            TO failed-travel.
        ENDLOOP.
      ENDIF.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<key>).
      TRY.
        ...
        CATCH cx_abap_foreign_lock INTO DATA(lx_foreign_lock).
          " fill failed return structure from framework
          APPEND VALUE #( travelid =  <key>-TravelId ) TO failed-travel.
          " fill reported structure to be displayed on the UI
          APPEND VALUE #( travelid = <key>-TravelId
  ...
  METHOD rba_Booking.
    DATA:
      legacy_parent_entity_out TYPE /dmo/travel,
    ...
    LOOP AT  keys_rba ASSIGNING FIELD-SYMBOL(<key_rba>) GROUP BY <key_rba>-TravelId.
      ...
      IF message IS INITIAL.
        ...
        ENDLOOP.
      ELSE.
      " fill failed table in case of errors
      failed-travel = VALUE #( BASE failed-travel FOR msg IN message
    ...
    LOOP AT entities_cba ASSIGNING FIELD-SYMBOL(<entity_cba>).
      ...
      IF messages IS INITIAL.
        ...
        LOOP AT <entity_cba>-%target ASSIGNING FIELD-SYMBOL(<entity>).
          ...
          IF messages IS INITIAL.
            INSERT VALUE #( %cid = <entity>-%cid travelid = travelid bookingid = entity-booking_id )
        ...
        ENDLOOP.
      ELSE.
        "fill failed return structure for the framework and what to be displayed on the UI
...
CLASS lsc_ZI_RAP_TRAVEL_U_FW DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI_RAP_TRAVEL_U_FW IMPLEMENTATION.
